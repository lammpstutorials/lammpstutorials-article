...
ensuring an initial temperature of $300\,\text{K}$ for these atoms. The \textit{fix nve}
...
\paragraph{Outputs}
Next, to measure the strain and stress suffered by the CNT, let us create a
...
variable F equal f_mysf2[1]-f_mysf1[1]
\end{verbatim}}
Here, the force is extracted from the fixes \textit{mysf1} and \textit{mysf2}
using \textit{f$\_$}, similarly to the use of \textit{v$\_$} to call variable,
and \textit{c$\_$} to call compute, as seen in \hyperref[lennard-jones-label]{tutorial 1}.
...
{\normalsize \begin{verbatim}
thermo 100
thermo_style custom step temp pe ke v_L v_F
...
\textit{Export YAML data to file}. Call the output \textit{log.yaml}.
To import the YAML log file into Python and plot the force $F$ as a function
of the CNT length $L$, download this script
\href{https://raw.githubusercontent.com/lammpstutorials/lammpstutorials-article/main/files/tutorial2/yaml-reader.py}{\textit{yaml-reader.py}} and save it within \textit{unbreakable-bonds/} 
The \textit{yaml-reader.py} first imports the \textit{log.yaml} file, and a certain pattern is
identified and stored as a string character named \textit{docs}. The string is
then converted into a list, and $F$ and $L$ are extracted into the list named \textit{F$\_$vs$\_$L},
which is saved in the data file named \textit{F$\_$vs$\_$L.dat} using the
...
then be used to plot the force $F$ as a function of the CNT length $L$, see
Fig.\,\ref{fig:CNT-stress-strai-unbreakable}.

\begin{figure}
\centering
\includegraphics[width=\linewidth]{CNT-stress-strain-unbreakable}
\caption{Force applied on the CNT during deformation, $F$, as a function of the
CNT length $L$.}
\label{fig:CNT-stress-strai-unbreakable}
\end{figure}
