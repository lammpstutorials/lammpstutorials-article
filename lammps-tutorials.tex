...
\begin{abstract}
Powerful and open-source molecular simulation codes allow users to launch molecular simulations without implementing inter-particle interactions or integrating Newton's equations of motion. However, while launching and analyzing a molecular simulation is now accessible to all, the wide array of options and potential pitfalls in these open-source tools can complicate their effective use. LAMMPS is a versatile molecular simulation code designed for modeling particles in a broad range of materials science and computational chemistry applications. It is widely used for its ability to run simulations on large-scale parallel computing environments, making it ideal for complex and extensive simulations of atomic and molecular systems. This article introduces seven tutorials designed to make learning LAMMPS more accessible to new users. These tutorials, of increasing difficulty, cover the basics of running molecular simulations in LAMMPS with systems of varying complexities, including a simple fluid, a carbon nanotube, and a polymer solvated in water. Each tutorial is centered on a specific system and has its own goal. Advanced molecular simulations are addressed by the last three tutorials, which cover the use of a reactive force field, umbrella sampling, and grand canonical Monte Carlo.
\end{abstract}
