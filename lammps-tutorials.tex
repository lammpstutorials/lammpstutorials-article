...
However, if you prefer to have you editor buffer automatically saved
before launching a run or exiting LAMMPS-GUI, there is a setting in the
\textit{Preferences} dialog for it.
...
\textit{Atoms} section depends on the choice of \textit{atom\_style}
and thus which per-atom data is set and stored internally in LAMMPS.
...
By visualizing the system using the generated JPG images, you may have
noticed that some atoms have moved from one region to the other during
...
important to avoid that the system will start to drift or move as a
whole.  For a bulk system with periodic boundary conditions, it is
expected to stay in place and thus when computing the temperature from
...
due to the drift, which means the system itself becomes cooler.  In the
extreme case it can freeze and drift very fast and thus people sometimes
refer to this as the ``flying ice cube syndrome''.

Run \textit{input.md.lmp} using LAMMPS, and observe the mixing of the
two populations over time (see also
Fig.\,\ref{fig:evolution-population}).  From the variables
\textit{n1\_in} and \textit{n2\_in}, one can follow the number of atoms
in each region as a function of time (Fig.\,\ref{fig:mixing}\,a). In
addition, as the mixing progresses, the coordination number between
atoms of types 1 and 2 increases from about $0.01$ to $0.35$
