...
Now, we create a loop with 50 steps and progressively move the center of the
bias potential by increments of 0.2\,nm.  Add the following lines
...
from $-48\,\mathrm{\AA}$ to $50\,\mathrm{\AA}$.  For each value of $x_\text{des}$,
...
where $i$ ranges from 1 to 50.  Run the \flecmd{umbrella-sampling.lmp} file using LAMMPS.
...
from the website must be placed next to \lmpcmd{umbrella-sampling.lmp}.  To apply the WHAM algorithm to our
simulation, we need a \flecmd{metadata.dat} file containing:
\begin{itemize}
\item the paths to all the data files,
\item the values of $x_\text{des}$,
\item the values of $k$.
\end{itemize}
Download the \href{\filepath tutorial7/metadata.dat}{\dwlcmd{metadata.dat}} file
...
\begin{lstlisting}
./wham -49 51 50 1e-8 119.8 0 metadata.dat umbrella-sampling.dat
\end{lstlisting}
where -50 and 50 are the boundaries, 50 is the number of bins, 1e-8 is the tolerance,
