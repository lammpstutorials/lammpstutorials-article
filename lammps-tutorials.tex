...
\begin{tcolorbox}[colback=mylightblue!5!white,colframe=mylightblue!75!black,title=For LAMMPS-GUI users]
\hyperref[lennard-jones-label]{Tutorial 1} can be followed using the LAMMPS-GUI. Instructions for GUI users are written in these boxes that can be ignored by non-GUI users.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=mylightblue!5!white,colframe=mylightblue!75!black,title=For LAMMPS-GUI users]
Either open the \textit{input.lammps} file using LAMMPS-GUI, or simply open LAMMPS-GUI. In this later case, an empty buffer appear, in which LAMMPS commands can be written.
\end{tcolorbox}
...
create_atoms A random 1500 &
    341341 simulation_box
create_atoms B random 100 &
    127569 simulation_box
\end{verbatim}}
\noindent The first line, \textit{region simulation$\_$box (...)}, creates a region named \textit{simulation$\_$box} that is a block (i.e. a rectangular cuboid) that extends from -20 to 20 (no unit) along all 3 directions of space. The second line, \textit{create$\_$box 2 simulation$\_$box}, creates a simulation box based on the region \textit{simulation$\_$box} with \textit{2} types of atoms. The third line, \textit{labelmap (...)} assigns the label `A' to the first atom type, and the label `B' to the second atom type. The fourth line, \textit{create$\_$atoms (...)} creates 1500 atoms of type A randomly within the region \textit{simulation$\_$box}. The integer \textit{341341} is a seed that can be changed to create different initial conditions for the simulation. The fifth line creates 100 atoms of type B.
...
pair_coeff B B 0.5 3.0
\end{verbatim}}
The two first commands, \textit{mass (...)}, attribute a mass equal to 1.0 (unitless) to atoms of both type A and B. The third line, \textit{pair$\_$coeff A A 1.0 1.0}, sets the Lennard-Jones coefficients for the interactions between atoms of type A, respectively the energy parameter $\epsilon_{A} = 1.0$ and the distance parameter $\sigma_{A} = 1.0$. Similarly, the last line sets the Lennard-Jones coefficients for the interactions between atoms of type 2, $\epsilon_{B} = 0.5$, and $\sigma_{B} = 3.0$. By default, LAMMPS calculates the cross coefficients between the different atom types using geometric average: $\epsilon_{ij} = \sqrt{\epsilon_{ii} \epsilon_{jj}}$, $\sigma_{ij} = \sqrt{\sigma_{ii} \sigma_{jj}}$.
...
lmp -in input.lammps
\end{verbatim}}
where the command \textit{lmp} is linked to a compiled version of LAMMPS.
...
\begin{tcolorbox}[colback=mylightblue!5!white,colframe=mylightblue!75!black,title=For LAMMPS-GUI users]
As the simulation runs, a windows showing the evolution of the temperature over time pops-up. Switch from the temperature to the potential energy by choosing \textit{PotEng} as the \textit{Select data}, or to the kinetic energy by choosing \textit{KinEng}.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=mylightblue!5!white,colframe=mylightblue!75!black,title=For LAMMPS-GUI users]
Generate an image of the system by selecting \textit{Create image} from the \textit{Run} menu.
\end{tcolorbox}
