...
\newtcolorbox{note}{
  colback=listing, % Background color (matches listings)
  colframe=listing, % Set frame color to match the background
...
dump mydmp all image 100 dump.md.*.ppm v_coor12 &
...
named \flecmd{parameters.inc}, as well as the scripts required for the second part
of the tutorial.
...
in a separate \lmpcmd{parameters.inc} file. This file must be placed within the same
...
\lmpcmd{rbot}. Add the following lines to \flecmd{input.lmp}:
\begin{lstlisting}
...
in each of the 3 regions by adding to \flecmd{input.lmp}:
\begin{lstlisting}
...
% SG: we could change it, yes. In the original version, it was also a pretext to
% make an exercice on the the flying ice cube... Its not shere anymore.
...
% so that , and a \lmpcmd{Berendsen} thermostat is applied
% to the atoms of the group \lmpcmd{cnt\_mid} only \cite{berendsen1984molecular}.
% AK This is incorrect. Any thermostat will thermostat only the group
% SG: OK. Shouln'd we remove the fix_modify then?
% and define its internal temperature compute for the same group.
%The \textit{fix\_modify myber} ensures that the \textit{fix Berendsen} uses the
%temperature of the group \textit{cnt\_mid} as an input, instead of the
...
% SG: I removed Berendsen now --> use it as a "Experiments" + cite \cite{berendsen1984molecular}.
...
dump mydmp all image 500 dump.*.ppm mol type &
  adiam 1.5 box no 0.01 view 0 90 zoom 1.8 shiny 0.1 fsaa yes
dump_modify mydmp pad 5 amap 0 10 sa 1 3 ${colors} &
  backcolor white
\end{lstlisting}
...
\lmpcmd{fix setforce}. Add the following lines in the \lmpcmd{input.lmp} file,
...
Open the following input named \lmpcmd{breakable.lmp} that must have
...
run 0 post no
\end{lstlisting}
Here, the \lmpcmd{metal} units system is used instead of \lmpcmd{real}, which is
