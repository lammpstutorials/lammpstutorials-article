...
\definecolor{note}{rgb}{0.741,0.925,1.00}
...
In \lmpcmd{breakable.lmp}, replace the \guicmd{run 0 post no} line with:
\begin{lstlisting}
...
Looking at the evolution of the energy, one can see that the total energy $E_\text{tot}$ is initially
...
The following input file \href{\filepath tutorial2/breakable.lmp}{\dwlcmd{breakable.lmp}}
contains a trick to represent bonds while using AIREBO. While it is beyong the scope
of the present tutorial, in short, the trick is to use AIREBO with the molecular atom
style, and use the \lmpcmd{bond/break} command to update the status of the bonds
during the simulation:
\begin{lstlisting}
fix break all bond/break 1000 1 2.5
\end{lstlisting}
% S.G.: we could probably write a bit more about it
