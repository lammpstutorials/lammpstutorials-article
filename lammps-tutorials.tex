...
The system is now fully parametrized and the input ready to compute
...
by reducing the \textit{GUI update interval} value.  At values lower
than 10ms the time the GUI spends on collecting the data becomes
significant and thus those should only be used for fast tutorials like
...
current system using \textit{Image Viewer} window.  To open it, use
either \textit{Create Image} from the \textit{Run} menu, using the
...
with \textit{run 0}.  This will do one force iteration in LAMMPS without
...
using the minimized geometry.  Since we want to start from the result
...
yes, as the former is falling while the latter is rising.  If you extend
the run to run for many more steps, the values for both kinetic and
...
throughout the run (the two numbers stand for he target at the beginning
and at the end of the run and which allow to have a temperature ramp, if
...
thermostat the atoms are subject friction and random noise (in the form
...
minimization (see also Fig.\,\ref{fig:evolution-energy}\,a). Then, after
...
This command tells LAMMPS to generate NetPMB format images every 100
