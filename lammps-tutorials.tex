...
This suite of tutorials is made of seven tutorials that are ordered by increasing difficulty. 

Tutorial 1, \hyperref[lennard-jones-label]{Lennard Jones fluid}, is meant for LAMMPS absolute beginners and requires no prior knowledge of LAMMPS. The structure of LAMMPS input files is presented. Basic LAMMPS commands are used to create a simple fluid, and perform energy minimization and molecular dynamics.

In \hyperref[carbon-nanotube-label]{tutorial 2}, the case of atoms connected by bonds is illustrated through the study of a carbon nanotube. The . The tutorial illustrates the difference between classical force field and {\color{red}a reactive force field called AIREBO}.

In \hyperref[all-atoms-label]{tutorial 3}, the solving of electrostatics interactions for dense systems will be illustrated through .

In \hyperref[sheared-confined-label]{tutorial 4}, an interface system is simulated, and the specifics of simulating a system made of both fluid and solid parts are presented.

In \hyperref[reactive-silicon-dioxide-label]{tutorial 5}, the reactive force field called \textit{ReaxFF} is used in conjunction with {\color{red}the charge equilibration method (QEq)}. 

In \hyperref[gcmc-silica-label]{tutorial 6}, a Monte Carlo simulation in the Grand Canonical ensemble is implemented to illustrate the use of LAMMPS with open systems that can exchange particles with a reservoir.

In \hyperref[umbrella-sampling-label]{tutorial 7}, an advanced free energy method named umbrella sampling is used. 
