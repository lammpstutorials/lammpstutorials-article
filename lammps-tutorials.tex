...
This suite of tutorials is made of seven tutorials that are ordered by increasing difficulty. The first tutorial, \hyperref[lennard-jones-label]{Lennard Jones fluid},
is meant for LAMMPS absolute beginners. The complexity of the simulations is then progressively increased for \hyperref[carbon-nanotube-label]{Pulling on a carbon nanotube}, \hyperref[all-atoms-label]{Polymer in water}, \hyperref[sheared-confined-label]{Nanosheared electrolyte}, and \hyperref[reactive-silicon-dioxide-label]{Reactive silicon dioxide}.
Finally, in \hyperref[gcmc-silica-label]{Water adsorption in silica} and
\hyperref[umbrella-sampling-label]{Free energy calculation},
some more advanced simulation methods are used, namely grand canonical Monte Carlo simulations and a free energy method named umbrella sampling.
...
\item EXTRA-DUMP
\end{itemize}
In addition, to visualize the atoms trajectories, the version 1.9.3 of VMD will be used \cite{humphrey1996vmd}. {\color{red}If you prefer, feel free to use an alternative visualization software like Ovito.} Finally a graph plotting tool is required. To plot the results from the simulations, the version 3.5.2 of Matplotlib Pyplot is used
together with the version 3.11.4 of Python \cite{van1995python, hunter2007Matplotlib}, but any plotting tool can be used. 
...
\caption{Snapshot of the silicon dioxide structure with colored charges.}
\label{fig:SIO}
\end{figure}
...
\caption{Snapshot of the water molecules adsorbed in cracked silica SiO$_2$ material.}
\label{fig:GCMC}
\end{figure}
...
\caption{Snapshot of the atoms. The atom colored in pink feels the additional force used for the umbrella sampling method.}
\label{fig:US}
\end{figure}
