...
density in \textcolor{blue}{kg/m$^3$}:  The variable \lmpcmd{myoxy} represents the number of
atoms divided by 3,  which corresponds to the number of molecules, $N_\text{H2O}$,
and the variable \lmpcmd{myrho} is the density in \textcolor{blue}{kg/m$^3$}:
\begin{equation}
\rho = \dfrac{N_\text{H2O} M}{V N_\text{A}},
\end{equation}
where $V$ is the volume in $\text{m}^3$, $N_\text{A}$ the Avogadro number, and
...
value of $1.472\,\text{kcal/mol}$ was used for $\epsilon_\text{1-5}$,
