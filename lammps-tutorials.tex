...
\begin{note}
  For the sake of simplicy we will refer during the tutorials to
...
launch LAMMPS--GUI, copy-n-paste the text below into the \textit{Editor}
window, and save it to the folder as a file called \textit{input.lmp}.
{\normalsize
\begin{verbatim}
...
\textsf{Ctrl--S} keyboard shortcut or by clicking on the ``Save'' icon
...
The system is now fully parametrized and ready to compute forces. Let us
fill in the two last remaining categories, \textit{Visualization}, and
\textit{Run}, by adding the following lines into \textit{input.lmp}:
{\normalsize
\begin{verbatim}
# 4) Visualization
thermo 10
...
\textsf{Ctrl-Enter} keyboard shortcut or by clicking on the green ``Run
...
\textit{Run} menu, the \textsf{Ctrl-/} keyboard shortcut or by clicking
...
If the plot in your \textit{Chart} window is missing data, then your
computer is running the simulation too fast for LAMMPS--GUI to collect
all the data.  You can adjust the settings in the \textit{Edit ->
  Preferences} dialog to collect data more frequently by reducing the
\textit{GUI update interval} value.  At values lower than 10ms the time
the GUI spends on collecting the data becomes significant and thus those
should only be used for fast tutorials like this one.  You can also make
your simulation slower by slecting \textit{None} in the
\textit{Accelerators} tab.
...
energy rapidly decreases and reaches a negative value, indicating that
the atoms have been displaced at reasonable distances from each other.

Since the \textit{thermo\_style} command also included the output of the
