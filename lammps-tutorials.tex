...
LAMMPS requires the users to provide detailed input files that can be complex to write, particularly considering the overwhelmingly large numbers of options offered by LAMMPS, all contained in an extensive {\color{red}documentation} that can sometimes be difficult to navigate for new users. In addition to the intrinsic complexity of LAMMPS, performing accurate molecular simulations involves several complex and system-specific decisions that must be made by the users, such as the selection of the thermodynamics ensemble (e.g. micro-canonical, grand-canonical) or the choosing of the most appropriate set of parameters to describe the interaction between the atoms (the so-called force field) \cite{van2018validation}.
...
The commands that must be written in a LAMMPS input file, as well as the commands that must be written in the terminal to launch LAMMPS and VMD are written in a {\normalsize \verb+verbatim+} environment. File and folder names are written in \textit{italic}.

\subsection{Software/system requirements}

The 2Aug2023 version of LAMMPS is required to follow the tutorials \cite{thompson2022lammps}. Download and install the 2Aug2023 version of LAMMPS by following the instructions of the {\color{red}LAMMPS website}. Depending on your operative system (i.e. Linux, macOS, or Windows), the procedure may differ. LAMMPS must be compiled with the following packages:
\begin{itemize}
\item MANYBODY
\item MOLECULE
\item KSPACE
...
\noindent The objective of this tutorial is to perform a simple MD simulation using LAMMPS. The system is a Lennard-Jones fluid made of neutral particles with two different diameters in a cubic box with periodic boundary conditions (Fig.\,\ref{fig:binary_LJ_fluid}). In this tutorial, the temperature of the system is maintained using a Langevin thermostat \cite{schneider1978molecular}, and basic quantities are extracted from the system, including the potential and kinetic energies. 
