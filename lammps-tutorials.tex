...
\begin{abstract}
The continuous development of highly powerful and user-friendly open-source codes makes it possible for any researcher, student, or curious-minded person, to launch a successful molecular simulations without implementing himself the inter-particle interaction or  integrate Newton's equations of motion. If the possibility of quick results is appealing, the access of these open-source codes remain hindered by their numerous options and possibilities, and the many pitfalls that comes with it. LAMMPS is a highly versatile molecular simulation code designed for modeling particles in a broad range of materials science and computational chemistry applications. It is widely used for its ability to run simulations on large-scale parallel computing environments, making it suitable for complex and large-scale simulations of atomic and molecular systems. This article introduces seven tutorials designed to make learning LAMMPS more accessible to new users. These tutorials of increasing difficulty cover the basics of running molecular simulations in LAMMPS with systems of varying complexities, including a simple fluid, a carbon nanotube, and a polymer solvated in water. Each tutorial has its own goal, such as imposing a deformation on the system or measuring a free energy barrier. Some more advanced topics are addressed by the last three tutorials, namely the use of reactive force fields, an illustration of the umbrella sampling method, and an example of grand canonical Monte Carlo simulation.
\end{abstract}
...
let us randomly delete some of the carbon atoms. To avoid deleting atoms that are too close to the edges, let us define a new region name \textit{rdel} that starts $2\,Å$ from the CNT edges.
{\normalsize \begin{verbatim}
...
The length $L$ of the CNT increases linearly over time for $t > 5\,\text{ps}$ (Fig.\,\ref{fig:CNT-unbreakable-lenght}), as expected from the imposed constant velocity. The energy, which can be accessed from the log file, shows a non-linear increase with $t$ once the deformation starts, which is expected from the typical dependency of bond energy with bond distance $U_b = k_b \left( r - r_0 \right)^2$ (Fig.\,\ref{fig:CNT-unbreakable-energy}).
...
In this tutorial, the water reservoir is first prepared in the absence of the polymer. A rectangular box of water is created and
...
include ../PARM.lammps
\end{verbatim}}
Download the \href{https://lammpstutorials.github.io/lammpstutorials-inputs/level2/polymer-in-water/mergePEGH2O/PEG-GROMOS.mol}{template} for the PEG molecule, and then create a single molecule in the middle of the box:
{\normalsize \begin{verbatim}
...
special_bonds lj 0.0 0.0 0.5 &
    ...
    angle yes dihedral yes
\end{verbatim}}
Start the simulation from the equilibrated PEG-water system and include again the parameter file by adding the following lines into the \textit{input.lammps}:
{\normalsize \begin{verbatim}
...
dump mydmp all atom 1000 dump.lammpstrj
\end{verbatim}}
Let us use a single Nosé-Hoover thermostat applied to all the atoms by adding the following lines into \textit{input.lammps}:
{\normalsize \begin{verbatim}
timestep 1.0
...
by adding the following lines into \textit{input.lammps}:
{\normalsize \begin{verbatim}
variable mytemp equal temp
fix myat1 all ave/time 10 10 100 &
...
run 30000
\end{verbatim}}
Run the \textit{input.lammps} file using LAMMPS. If you open the \textit{dump.lammpstrj} file using \textit{VMD}, you should see that the PEG molecule eventually aligns in the direction of the force (as seen in Fig.\,\ref{fig:PEG-in-water}). The evolution of the end-to-end distance over time shows the PEG adjusting to the external forcing (Fig.\,\ref{fig:PEG-distance}).
...
\caption{a) Evolution of the length $L$ of the CNT with time. The CNT starts deforming at $t = 5\,\text{ps}$. b) Evolution of the gyration radius $R_\text{gyr}$ of the PEG molecule.}
...
Let us create the box by adding the following lines into \textit{input.lammps}:
{\normalsize \begin{verbatim}
lattice fcc 4.04
...
necessary information concerning the water molecule, such as atom positions, bonds, and angles. Add the following lines into \textit{input.lammps}:
{\normalsize \begin{verbatim}
...
\end{verbatim}}
To avoid high density and pressure, let us add the following lines into \textit{input.lammps} to delete a few of the water molecules:
{\normalsize \begin{verbatim}
