...
Molecular simulations can be used to model and analyze the behavior of atomic or coarse-grained systems. These simulations can provide insights into both structural and dynamical properties of a large variety of systems that are relevant to biology, physics, or materials science. These systems can be as diverse as (bio)molecules, polymers, solids, and colloidal suspensions. Molecular simulations regroup different methods such as molecular dynamics for which the Newton equations of motion are solved iteratively to predict the evolution of the system, and Monte Carlo simulations for which certain operations are performed randomly on the system and accepted/rejected based on energy criteria, allowing for efficient exploration of the configuration space. Molecular simulations allow for the measurement of a broad variety of properties including structural properties (e.g. bond length, inter-atomic distances), thermodynamic properties (e.g. temperature, pressure, volume), dynamic behaviors (e.g. diffusion coefficient, viscosity), or mechanical responses (e.g. elastic constant, Poisson ratio). Some of these quantities can be directly compared with experimental data, allowing for the direct validation of the simulated system, while the additional information that can be extracted from simulations is often useful to interpret experimental data. 

LAMMPS (Large-scale Atomic/Molecular Massively Parallel Simulator) is a highly flexible open-source molecular dynamics tool \cite{thompson2022lammps}. Over the years, a broad variety of particle interaction models have been implemented in LAMMPS, enabling it to model atomic, polymeric, biological, metallic, granular, and coarse-grained systems. If LAMMPS can run on a single CPU core, it also shows high performance on parallel supercomputing systems and can handle complex and large-scale simulations efficiently.
