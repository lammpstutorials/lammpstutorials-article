...
\begin{abstract}
Highly powerful and open-source molecular simulation codes enable any user to launch a successful molecular simulation without personally implementing inter-particle interactions or integrating Newton's equations of motion. While the possibility of quick results is appealing, access to these open-source codes remains hindered by their numerous options and possibilities, as well as the many pitfalls that come with them. LAMMPS is a highly versatile molecular simulation code designed for modeling particles in a broad range of materials science and computational chemistry applications. It is widely used for its ability to run simulations on large-scale parallel computing environments, making it suitable for complex and large-scale simulations of atomic and molecular systems. This article introduces seven tutorials designed to make learning LAMMPS more accessible to new users. These tutorials, of increasing difficulty, cover the basics of running molecular simulations in LAMMPS with systems of varying complexities, including a simple fluid, a carbon nanotube, and a polymer solvated in water. Each tutorial is centered on a specific system and has its own goal. Advanced molecular simulations are addressed by the last three tutorials, which cover the use of a reactive force field, umbrella sampling, and grand canonical Monte Carlo.
\end{abstract}
