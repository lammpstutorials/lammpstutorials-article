...
\begin{abstract}
  ...
  % AK: ideally, there would be an eigth tutorial showcasing both
...
biomolecules, as well as complex interfaces and multicomponent systems.
...
of features and options offered by LAMMPS.  While LAMMPS has an
...
\cite{van2018validation}.  These choices are independend from the chosen
...
a simple MD simulation in the canonical (NVT) ensemble.  This tutorial
also illustrates the basic use of the LAMMPS graphical user interface
(LAMMPS--GUI) for writing input files, running simulations, and
visualizing results.
...
demonstrates the use of molecule files and typelabels
...
(QEq).  This tutorial describes a situation where the partial charges of
the atoms adapt to their local environment and demonstrates how to
visualize it.
...
% S.G. Should we also tell the user to reduce the number of CPU-cores? I've noticed that
% by default the lammps-gui tries to use as many CPU-cores as possible.
