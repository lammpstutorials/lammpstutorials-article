...
select a folder, create one if needed, and place several files into it.
...
CNT topology and geometry, named \flecmd{unbreakable.data}, as well as scripts
...
of the forces applied to the atoms.  These bonds typically consist of springs
with equilibrium distances $r_0$ and force constants $k_\text{b}$:
$U_\text{b} = k_\text{b} \left( r - r_0 \right)^2$.  Additionally, angular and
dihedral constraints are often applied to preserve the molecular structure
...
\lmpcmd{atom\_style} is molecular (therefore atoms are point particles
...
\lmpcmd{Atoms} header line in the data files indicating the intended
...
every 500 steps:
\begin{lstlisting}
dump mydmp all image 500 dump.*.ppm type type &
