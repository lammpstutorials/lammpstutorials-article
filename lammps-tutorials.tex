...
carbon nanotube (CNT) in an empty box (Fig.\,\ref{fig:CNT}).  An
% FIXME: not a force but strain.
external force is imposed on the CNT, and its deformation is monitored
over time.  To illustrate the difference between classical and reactive
force fields, this tutorial is divided into two parts.  In the first
part, a classical force field \cite{typelabel_paper} is used and the
bonds between the atoms of the CNT are unbreakable.  In the second part,
a reactive force field (named AIREBO \cite{stuart2000reactive}) is used,
allowing for the breaking of chemical bonds when the CNT experiences
large strain from the deformation.
...
\textit{cnt\_molecular.data} topology and geometry, and a file
\textit{parameters.inc} with force field parameters.
...
After following the setup the editor should have the following content:
{\normalsize
\begin{verbatim}
...
read_data cnt_molecular.data extra/atom/type 1
\end{verbatim}
}
The chosen unit system is \textit{real} (therefore distances are in
Ångstrom and time in femtosecond), the \textit{atom\_style} is molecular
(therefore atoms are point particles that can be bonded with each
other), and the boundary conditions are fixed.   The boundary conditions
do not matter here, as the box boundaries were placed far from the
CNT.  Just like in the previous tutorial,
\hyperref[lennard-jones-label]{Lennard-Jones fluid}, the pair style is
\textit{lj/cut} (i.e.~a Lennard-Jones potential with a cutoff) with
cutoff set to 14 Ångstroms, which means that only the atoms closer than
that interact through the Lennard-Jones potential.
...
\textit{cnt\_molecular.data} file which contains information about the
box size, atom positions, as well as the identity of the atoms that are
...
\cite{kohlmeyer2017topotools}.  Since TopoTools knows nothing about
force fields the data file does not contain force field parameters
(unlike the data file we created with LAMMPS in tutorial 1), and thus we
need to specify the parameters of both, the bonded and non-bonded
potentials separately.

The parameters we use are taken from the OPLS-AA (Optimised Potentials
for Liquid Simulations-All-Atom) force field
\cite{jorgensenDevelopmentTestingOPLS1996}.  These are stored in the file
\textit{parameters.inc} which should contain: {\normalsize
\begin{verbatim}
...
% AK: the english here reads a bit "quirky". Jake, any ideas as anative speaker?
...
set group carbon_top mol 1
set group carbon_bot mol 2
set group carbon_mid mol 3
\end{verbatim}
}
...
% AK: why temp/berendsen? This seems particularly unsuitable for this system
% it is designed for bulk systems that are in equilibrium, so that you
% can do an "almost" NVE while removing extra kinetic energy from noise.
The \textit{fix nve}
...
\textit{carbon\_mid} only \cite{berendsen1984molecular}.
% AK This is incorrect. Any thermostat will thermostat only the group
% and define its internal temperature compute for the same group.
%
%The
