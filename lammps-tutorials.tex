...
\textit{systemcreation/}, open a blank file called \textit{input.lammps}, and
...
two regions. Add the following lines to \textit{input.lammps}:
{\normalsize \begin{verbatim}
...
bonds, and angles. Add the following lines to \textit{input.lammps}:
{\normalsize \begin{verbatim}
...
the water molecules, by adding the following commands to \textit{input.lammps}:
{\normalsize \begin{verbatim}
...
\end{verbatim}}
Create a new text file called \textit{PARM.lammps} next to the \textit{systemcreation/}
folder. Copy the following lines into \textit{PARM.lammps}:
{\normalsize \begin{verbatim}
...
the solid walls would be extremely hydrophilic, causing the water molecule to
...
hydrophilic by reducing the value of $\epsilon_\text{1-5}$. Copy the following
...
Let us also create another file called \textit{GROUP.lammps} next to
\textit{PARM.lammps}, and copy the following lines into it:
{\normalsize \begin{verbatim}
group H2O type 1 2
group Na type 3
group Cl type 4
...
high density and pressure, let us add the following lines into \textit{input.lammps}
...
an image of the system. Run the \textit{input.lammps} file
using LAMMPS. Check that your system has been correctly created by also looking
at the periodic images, just like in Fig.\,\ref{fig:NANOSHEAR-system}.
...
and create a new input file called \textit{input.lammps} in it. Copy the following lines
...
Let us also print the atom positions in a \textit{.lammpstrj} file and control
the printing of thermodynamic outputs by adding the following lines to \textit{input.lammps}:
{\normalsize \begin{verbatim}
dump mydmp all atom 1000 dump.lammpstrj
thermo 200
\end{verbatim}}
...
\textit{nve/limit}, which will allow for a better relaxation of the atom positions.

When running the \textit{input.lammps} file with LAMMPS, you should see that the
total energy of the system decreases during the first of the 3 steps, before
re-increasing a little after the temperature is increased from 1 to $300\,\text{K}$
(Fig.\,\ref{fig:NANOSHEAR-minimization}). If you look at the trajectory using VMD,
...
next to the previously created folders, and create a new \textit{input.lammps}
file in it. Add the following lines into \textit{input.lammps}:
{\normalsize \begin{verbatim}
boundary p p f
units real
atom_style full
...
visualization easier. Then, add the following lines into \textit{input.lammps}
...
write_data system.data nocoeff
\end{verbatim}}
Run the \textit{input.lammps} file using LAMMPS. As seen from the data printed by
...
Finally, let us dump the atom positions, extract the velocity profiles using
several \textit{ave/chunk} commands, extract the force applied on the walls, and
then run for $200\,\text{ps}$ Add the following lines into \textit{input.lammps}:
{\normalsize \begin{verbatim}
...
A.K. acknowledges financial support by Sandia National Laboratories under POs 2149742 and 2407526.
